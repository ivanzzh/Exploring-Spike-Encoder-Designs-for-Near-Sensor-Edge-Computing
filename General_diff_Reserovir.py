...
min_gradient_record = []
RESERVOIR_MAP = {
    "random_sparse": StepByStepRandomSparseReservoir,
    "cyclic": StepByStepCyclicReservoir,
    "hierarchical": StepByStepHierarchicalReservoir,
    "modular": StepByStepModularReservoir,
    "distance_constrained": StepByStepDistanceConstrainedReservoir,
}

RESERVOIR_CONFIGS = {
    "random_sparse_example": {
        "reservoir_name": "random_sparse",
        "input_size": input_size,
        "output_size": r_output,
        "sparsity": 0.1
    },
    "cyclic_example": {
        "reservoir_name": "cyclic",
        "input_size": input_size,
        "output_size": r_output
    },
    "hierarchical_example": {
        "reservoir_name": "hierarchical",
        "input_size": input_size,
        "output_size": r_output,
        "num_layers": 3,
        "scale_factor": 10,
    ...
    "modular_example": {
        "reservoir_name": "modular",
        "input_size": input_size,
        "output_size": r_output
    },
    "distance_constrained_example": {
        "reservoir_name": "distance_constrained",
        "input_size": input_size,
        "output_size": r_output,
        "distance_decay": 0.5
    }
...
def train(Res, snn1, snn2, snn3, train_dataloader, criterion, optimizer, device, epoch, point_array):
    ...
    for i_batch, sample_batched in enumerate(train_dataloader):
        ...
        for i in range(window_length):
            ...
            reservoir_output = Res(x_train[:, i, :])
...
def call_reservoir(train_loader, test_loader, reservoir_name, input_size, output_size, **kwargs):
    if torch.cuda.is_available():
        device = torch.device('cuda')
    else:
        device = torch.device('cpu')
    reservoir_class = RESERVOIR_MAP[reservoir_name]
    Res = reservoir_class(input_size, output_size, **kwargs)
    ...
    for epoch in range(epochs):
        last_record, max_index, avg_spike_count = train(Res, snn1, snn2, snn3, train_loader, criterion, optimizer, device, epoch,
                                       point_array)
    ...
    lines = 6
    col = int(len(max_gradient_record)/lines)
    i = 1
    for gradient in max_gradient_record:
        print('\t{}'.format(gradient), end='')
        if i % col == 0:
            print()
        i += 1
    i = 0
    while 1:
        filename = '{}_{}_{}'.format(reservoir_name, name, i)
    ...
    image_name = '{}_Res_Schedule_Eprop_{}_{}_class_{:.4f}.jpg'.format(name, window_length, layer3_neuron_number, best_acc)
    image_path = os.path.join(path, image_name)
    plt.plot(epoch_list, acc_record, color='red', linewidth=2.0)
    plt.xlabel('epoch')
    plt.ylabel('acc')
    plt.title('{} {} Res SOLSA best accuracy: {}'.format(name, reservoir_name, best_acc), fontsize='large')
...
def main(r_name_lsit):
    ...
    acc_dict = {}
    for r_name in r_name_lsit:
        if r_name not in RESERVOIR_CONFIGS:
            raise ValueError(f"Example {r_name} not found in RESERVOIR_CONFIGS")
        config = RESERVOIR_CONFIGS[r_name]
        acc = call_reservoir(**config, train_loader=train_loader, test_loader=test_loader)
        acc_dict[r_name] = acc

    for key, value in acc_dict.items():
        print(f"the best accuracy for {key} training: {value}")
